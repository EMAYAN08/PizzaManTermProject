...
function App(props) {
  useEffect(() => {
    ...
    if (currentUser) {
      // If there is a current user, dispatch the authChangedHandler
      props.authChangedHandler(currentUser);
    } else {
      // If there is no current user, dispatch the authChangedHandler with null
      props.authChangedHandler(null);
    }
  }, [props])
